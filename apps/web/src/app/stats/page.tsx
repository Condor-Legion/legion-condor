...
export default function PublicStatsPage() {
  ...
  useEffect(() => {
    ...
    async function loadStats(): Promise<void> {
      ...
      try {
        const response = await fetch(`${apiUrl}/api/stats/public-report`);
  ...
  const filteredRows = useMemo(() => {
    ...
    const source = normalizedQuery.length === 0
      ? rows
      : rows.filter((row) => {
          return row.displayName.toLowerCase().includes(normalizedQuery);
        });
        ...
        <header className="flex flex-col gap-4 border-b border-neutral-800/80 pb-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-neutral-300">
              Ranking histórico de miembros activos basado en sus estadísticas de partidas, con comparación directa entre jugadores.
            </p>
          </div>
